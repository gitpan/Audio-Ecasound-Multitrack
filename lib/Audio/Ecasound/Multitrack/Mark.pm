...
sub new {
	...
	push @all, $object;
	$Audio::Ecasound::Multitrack::this_mark = $object;
...
sub jump_here {
	...
	carp("you attempted to futz $mark, which is not a Mark\n"),
		return unless (ref $mark) =~ /Mark/;
	Audio::Ecasound::Multitrack::eval_iam( "setpos " . $mark->time);
	$Audio::Ecasound::Multitrack::this_mark = $mark;
}
sub remove {
	my $mark = shift;
	...
	$Audio::Ecasound::Multitrack::debug and warn "marks found: ",scalar @all, $/;
...
sub next { 
	...
	carp("you attempted to futz $mark, which is not a Mark\n"),
		return unless (ref $mark) =~ /Mark/;
	Audio::Ecasound::Multitrack::next_mark();
}
sub previous {
	my $mark = shift; 
	carp("you attempted to futz $mark, which is not a Mark\n"),
		return unless (ref $mark) =~ /Mark/;
	Audio::Ecasound::Multitrack::previous_mark();
}
...
sub loop_start { 
	my @points = sort { $a <=> $b } 
	grep{ $_ } map{ mark_time($_)} @Audio::Ecasound::Multitrack::loop_endpoints[0,1];
	#print "points @points\n";
	$points[0];
}
sub loop_end {
...
sub mark_time {
	...
	if ($tag =~ /\./) { # we assume raw time if decimal
		...
		$mark = $Audio::Ecasound::Multitrack::Mark::all[$tag];
	} else {
		#print "mark name found\n";
		$mark = $Audio::Ecasound::Multitrack::Mark::by_name{$tag};
	}
